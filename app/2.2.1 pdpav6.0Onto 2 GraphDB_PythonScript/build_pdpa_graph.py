...
* The PDF supplied is the "unofficial translation" and contains Sections 1-29,
  which matches my modeled scope (Chapters I-II). Sections referenced but out
  of scope (30, 31, ... 73) get :Section stub nodes with no text, exactly so my
...
def canon(name: str) -> str:
    """
    Single canonical form for every concept/edge name: underscore- and
    hyphen-free PascalCase. e.g. 'Personal_Data' -> 'PersonalData',
    'Cross-border_Transfer' -> 'CrossBorderTransfer'. Applied uniformly to nodes
    AND edge endpoints so the graph is internally consistent regardless of the
    underscores in the source .ttl. Section numbers are keyed on integers, not
    names, so they are unaffected.
    """
...
def parse_ontology(ttl_path: str) -> Model:
        ...
        for parent in g.objects(cls, RDFS.subClassOf):
            if isinstance(parent, URIRef):
                ...
                if on_prop is not None and some_val is not None:
                    m.requires.append(
                        (name, canon(local_name(some_val)), local_name(on_prop))
                    )
    ...
    for prop in g.subjects(RDF.type, OWL.ObjectProperty):
        ...
        if not (isinstance(dom, URIRef) and isinstance(rng, URIRef)):
            continue
        dname, rname = canon(local_name(dom)), canon(local_name(rng))
